...
## Future: 3D Voxel Cave

The current generator is 2D-per-slice with stair teleports between floors. For
true 3D flight, it will be redesigned as a full voxel generator:

### Changes

| Aspect | Current | Future |
|---|---|---|
| Grid | `[floor][row][col]` tiles | `[z][y][x]` voxels |
| Vertical | Discrete stairs, teleport | Ramps, sloped passages, open vertical shafts |
| Connectivity | Per-floor flood fill + stair bridges | 3D 26-direction flood fill |
| Movement | 4-direction per floor | Full 3D (x, y, z) |
| D\* Lite / A\* | 2D cost grid | 3D cost grid with 26 neighbours |
| Gazebo output | 2D wall meshes per floor | 3D wall mesh from voxel surface |

### Algorithm

The same cellular automata pipeline applies in 3D: random fill → 3D Moore
neighbourhood smoothing (7 neighbours threshold) → region connection → dead-end
carving. Ramps are generated by carving sloped transitions between adjacent
elevation layers.

### Impact

- **Pathfinding** — D\* Lite and A\* move from 2D to 3D (26-connected grid)
- **SLAM** — 2D occupancy grid → 3D occupancy / point cloud
- **Gazebo world** — SDF mesh from Marching Cubes or similar voxel-to-mesh
  conversion
